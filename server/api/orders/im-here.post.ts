...
export default defineEventHandler(async (event) => {
  ...
  if (existingArrivedAt) {
    return {
      ...
      pickup_arrived_at: existingArrivedAt,
    };
  }
  ...
  const { data: updated, error: updateError } = await (admin as any)
    ...
    .eq("id", body.orderId)
    .select("id, metadata, order_number, store:store_id(name)")
    .single();
  ...
  // Send push notification for driver arrival
  try {
    await notifyUser(event, callerId, "driver_arrived", {
      orderId: body.orderId,
      orderNumber: updated?.order_number,
      storeName: updated?.store?.[0]?.name,
      status: "arrived",
      url: `/orders/${body.orderId}`,
    });
  } catch (notifyErr) {
    console.error(
      "[Push Notification] Failed to send driver arrival:",
      notifyErr,
    );
  }
