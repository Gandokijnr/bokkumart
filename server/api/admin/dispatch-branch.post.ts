...
export default defineEventHandler(async (event): Promise<BranchDispatchResponse> => {
  try {
    const body = await readBody<BranchDispatchRequest>(event)
    const { orderId, storeId, riderName, riderPhone, riderVehicleType, estimatedArrival, dispatchedBy } = body

    if (!orderId || !storeId || !riderName || !dispatchedBy) {
      throw createError({
        statusCode: 400,
        message: 'Missing required fields: orderId, storeId, riderName, dispatchedBy'
      })
    }
    ...
    if (!isSuperAdmin && allowedStoreIds && !allowedStoreIds.includes(storeId)) {
      throw createError({
        statusCode: 403,
        message: 'Access denied. You can only dispatch orders for your assigned branch.'
      })
    }
