...
export default defineEventHandler(async (event) => {
  ...
  if (!storeId || !settlementBankCode || !accountNumber) {
    throw createError({ statusCode: 400, statusMessage: 'store_id, settlement_bank_code, account_number are required' })
  }
  ...
  if (storeErr || !storeRow) {
    throw createError({ statusCode: 400, statusMessage: storeErr?.message || 'Store not found' })
  }
  ...
  if (storeRow.paystack_subaccount_code) {
    return {
      ...
      message: 'Store already has a subaccount'
    }
  }

  const businessName = String(body?.business_name || storeRow.name || 'Store').trim()
  ...
  if (!psRes.ok || !psJson.status || !psJson.data?.subaccount_code) {
    ...
    throw createError({ statusCode: 400, statusMessage: psJson.message || 'Failed to create subaccount' })
  }

  const platformPercentage = typeof body?.platform_percentage === 'number'
    ? body.platform_percentage
    : null

  const fixedCommission = typeof body?.fixed_commission === 'number'
    ? body.fixed_commission
    : null
