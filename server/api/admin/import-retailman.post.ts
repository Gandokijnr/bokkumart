...
export default defineEventHandler(async (event) => {
  ...
  const supabaseUrl =
    (config.supabaseUrl as string | undefined) || process.env.SUPABASE_URL;
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    throw createError({
      statusCode: 500,
      statusMessage: `Server not configured for inventory upload (URL: ${supabaseUrl ? "OK" : "MISSING"}, Key: ${serviceRoleKey ? "OK" : "MISSING"})`,
    });
  }
  ...
  if (callerErr || !callerData?.user) {
    ...
    throw createError({
      statusCode: 401,
      statusMessage: `Invalid session: ${callerErr?.message || "Unknown error"}`,
    });
  }
    ...
    try {
      ...
      if (existingBySku) {
        ...
        const { error: updateError } = await (admin as any)
          ...
          .update({
            price: row.retail,
            name: row.details || existingBySku.name,
