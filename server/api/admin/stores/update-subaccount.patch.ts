...
export default defineEventHandler(
  async (event): Promise<UpdateSubaccountResponse> => {
    try {
      ...
      const { data: callerData, error: callerErr } = await (
        supabase as any
      ).auth.getUser(token);
      ...
      if (profileErr || !callerProfile) {
        throw createError({
          statusCode: 401,
          message: profileErr?.message || "Unauthorized",
        });
      }

      const user = {
        id: callerId,
        role: String((callerProfile as any).role || ""),
        store_id: (callerProfile as any).store_id
          ? String((callerProfile as any).store_id)
          : null,
        full_name: String((callerProfile as any).full_name || ""),
        email: String((callerData.user as any).email || ""),
      };
      ...
      if (!isSuperAdmin && !isBranchManager) {
        throw createError({
          statusCode: 403,
          message:
            "Only super admin or branch manager can update subaccount settings",
        });
      }
      ...
      if (paystackSubaccountCode !== undefined) {
        ...
        if (
          paystackSubaccountCode &&
          !paystackSubaccountCode.startsWith("ACCT_")
        ) {
          throw createError({
            statusCode: 400,
      ...
      if (paystackSettlementBankName !== undefined) {
        updates.paystack_settlement_bank_name =
          paystackSettlementBankName || null;
      }
      ...
      if (paystackSettlementAccountNumber !== undefined) {
        ...
        if (
          paystackSettlementAccountNumber &&
          !/^\d{10}$/.test(paystackSettlementAccountNumber)
        ) {
          throw createError({
            statusCode: 400,
        ...
        updates.paystack_settlement_account_number =
          paystackSettlementAccountNumber || null;
      }
      ...
      const { data: updatedStore, error: updateError } = await supabase
        ...
        .eq("id", storeId)
        .select(
          "id, name, paystack_subaccount_code, paystack_settlement_bank_name, paystack_settlement_account_number, platform_percentage, fixed_commission",
        )
        .single();
