...
export default defineEventHandler(async (event) => {
  ...
  const nextRole = body.role ? String(body.role) : null;
  const storeId =
    body.storeId === null ? null : String(body.storeId || "").trim() || null;
  if (nextRole === "staff" && !storeId) {
    throw createError({
      statusCode: 400,
  ...
  const updateData: Database["public"]["Tables"]["profiles"]["Update"] = {
    ...
    store_id: body.storeId === undefined ? undefined : storeId,
    managed_store_ids: Array.isArray(body.managedStoreIds)
      ? body.managedStoreIds.length > 0
        ? body.managedStoreIds
        : null
  ...
  if (!error && body.role) {
    const { data: targetUserData, error: targetUserErr } = await (
      admin as any
    ).auth.admin.getUserById(body.id);
    if (targetUserErr) {
      throw createError({
        statusCode: 400,
    ...
    const { error: authUpdateErr } = await (
      admin as any
    ).auth.admin.updateUserById(body.id, {
      app_metadata: {
        ...existingAppMeta,
        role: body.role,
      },
