...
export default defineEventHandler(async (event) => {
  ...
  if (!isBranchManager && !isSuperAdmin) {
    throw createError({
      statusCode: 403,
      statusMessage: "Not authorized to manage inventory",
    });
  }
  ...
  if (!store_id || !name || stock_level === undefined) {
    throw createError({
      statusCode: 400,
      statusMessage: "Missing required fields: store_id, name, stock_level",
    });
  }
  ...
  if (allowedStoreIds !== null && !allowedStoreIds.includes(store_id)) {
    throw createError({
      statusCode: 403,
      statusMessage: "Not authorized to manage this store",
    });
  }
  ...
  // Try to find by barcode first (RetailMan)
  if (!productId && barcode) {
    const { data: existingByBarcode } = await (admin as any)
      .from("products")
      .select("id")
      .eq("barcode", barcode)
      .single();
    if (existingByBarcode) productId = existingByBarcode.id;
  }
  ...
  if (!productId && price) {
    const { data: newProduct, error: productError } = await (admin as any)
      ...
      .insert({
        ...
        retailman_product_id: retailman_product_id || null,
        price: price,
    ...
    if (!newProduct) {
      throw createError({
        statusCode: 500,
        statusMessage: "Failed to create product: no data returned",
      });
    }
  ...
  if (!productId) {
    throw createError({
      statusCode: 400,
      statusMessage:
        "Product not found. Provide a price to create a new product.",
    });
  }

  // Update product with RetailMan fields if provided (for existing products)
  if (productId && (barcode || retailman_product_id)) {
    await (admin as any)
      .from("products")
      .update({
        barcode: barcode || null,
  ...
  if (existingInventory) {
    ...
    if (updateError) {
      throw createError({
        statusCode: 500,
        statusMessage: `Failed to update inventory: ${updateError.message}`,
      });
    }
    action = "updated";
  } else {
    ...
    if (insertError) {
      throw createError({
        statusCode: 500,
        statusMessage: `Failed to create inventory: ${insertError.message}`,
      });
    }
  }
  ...
  try {
    await (admin as any).rpc("log_audit_action", {
      p_action_type:
        action === "created" ? "inventory_update" : "stock_adjustment",
      p_entity_type: "store_inventory",
      p_entity_id: productId,
      p_store_id: store_id,
      p_old_value: existingInventory
        ? { stock_level: existingInventory.stock_level }
        : null,
