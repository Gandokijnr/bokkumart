...
export default defineEventHandler(async (event) => {
  ...
  if (!body?.orderId || !desiredStatus) {
    throw createError({ statusCode: 400, statusMessage: 'orderId and status are required' })
  }

  const { data: existingOrder, error: fetchErr } = await (admin as any)
    .from('orders')
    .select('id, status, delivery_method, delivery_details, contact_phone, metadata, store_id, items')
    .eq('id', body.orderId)
    .single()

  if (fetchErr || !existingOrder) {
    throw createError({ statusCode: 400, statusMessage: fetchErr?.message || 'Order not found' })
  }
  ...
  if (updateErr || !updatedOrder) {
    throw createError({ statusCode: 400, statusMessage: updateErr?.message || 'Failed to update order' })
  }
    ...
    for (const item of items as any[]) {
      ...
      try {
        const { data: invRow, error: invErr } = await (admin as any)
          ...
          .select('id, stock_level, reserved_stock, available_stock, digital_buffer, is_visible')
          .eq('store_id', storeId)
