...
function parseCSV(content: string): ParseResult {
  ...
  if (lines.length < 2) {
    return { success: [], errors: [{ row: 0, message: 'CSV must have a header row and at least one data row' }] }
  }
  ...
  if (missingFields.length > 0) {
    return { 
      success: [], 
      errors: [{ row: 0, message: `Missing required columns: ${missingFields.join(', ')}` }] 
    }
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (isNaN(stockLevel) || stockLevel < 0) {
      errors.push({ row: i + 1, message: `Invalid stock_level: ${row.stock_level}` })
      continue
    }
    ...
    const storePrice = row.store_price ? parseFloat(row.store_price) : undefined
    const digitalBuffer = row.digital_buffer ? parseInt(row.digital_buffer) : 0

    if (row.cost_price && (isNaN(costPrice!) || costPrice! < 0)) {
      errors.push({ row: i + 1, message: `Invalid cost_price: ${row.cost_price}` })
      continue
    }
...
export default defineEventHandler(async (event) => {
  ...
  if (!isBranchManager && !isSuperAdmin && !isStaff) {
    throw createError({ statusCode: 403, statusMessage: 'Not authorized to upload inventory' })
  }
  ...
  const { data: categories } = await (admin as any).from('categories').select('id, name')
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      if (!productId) {
        results.errors.push({ row: i + 2, message: `Failed to create product: ${row.name || 'unknown'}. Check price and required fields.` })
        continue
      }
  ...
  try {
    await (admin as any).rpc('log_audit_action', {
      ...
      p_store_id: store_id,
      p_old_value: null,
      p_new_value: { csvRows: rows.length, created: results.created, updated: results.updated },
      p_description: `CSV inventory upload: ${rows.length} rows processed`,
      p_metadata: { fileName: file.filename, parseErrors: parseErrors.length > 0 ? parseErrors : null }
    })
  } catch (e) {
