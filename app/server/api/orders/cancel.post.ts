...
export default defineEventHandler(async (event) => {
  ...
  if (!['pending', 'processing'].includes(status)) {
    throw createError({ statusCode: 409, statusMessage: `Order cannot be cancelled (status: ${status})` })
  }
  ...
  const cancelledCount = (recentOrders || []).filter((o: any) => String(o?.status) === 'cancelled').length
  if (cancelledCount >= 5) {
    throw createError({ statusCode: 429, statusMessage: 'Too many cancellations. Please contact support.' })
  }
