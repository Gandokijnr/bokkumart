...
export default defineEventHandler(async (event) => {
  ...
  const contactPhone = normalizePhone(
    String((body as any)?.contact_phone || ""),
  );
  ...
  if (!storeId || !deliveryMethod || !contactName || !contactPhone) {
    throw createError({
      statusCode: 400,
      statusMessage: "Please complete your details and try again.",
    });
  }
  ...
  if (deliveryMethod === "pickup") {
    throw createError({
      statusCode: 400,
      statusMessage: "Pickup requires upfront payment",
    });
  }
  ...
  if (!isPhoneVerified(profile)) {
    throw createError({
      statusCode: 403,
      statusMessage: "Phone verification required for pay-on-delivery",
    });
  }
  ...
  if (isNewUser) {
    ...
    if (podCountErr) {
      throw createError({
        statusCode: 500,
        statusMessage: podCountErr.message,
      });
    }
    ...
    if ((podCount || 0) >= 1) {
      throw createError({
        statusCode: 403,
        statusMessage:
          "Pay-on-delivery is temporarily limited for new accounts. Please pay online.",
      });
    }
  }
  ...
  const cancels = (recentOrders || []).filter(
  ...
  if (cancels >= 3) {
    throw createError({
      statusCode: 403,
      statusMessage:
        "Pay-on-delivery is disabled due to repeated cancellations. Please pay online.",
    });
  }
