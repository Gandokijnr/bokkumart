...
export default defineEventHandler(async (event) => {
  ...
  if (!store_id || items.length === 0) {
    throw createError({ statusCode: 400, statusMessage: 'store_id and items are required' })
  }
  ...
  const invMap = new Map<string, { available_stock: number; digital_buffer: number; is_visible: boolean }>()
