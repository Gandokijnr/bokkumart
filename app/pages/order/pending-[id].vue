      ...
      <div v-else-if="order.status === 'awaiting_call'" class="space-y-6">
        ...
        <div class="rounded-2xl bg-white p-6 shadow-sm">
          <div class="mb-4 flex items-center justify-center">
            <div
              class="flex h-20 w-20 items-center justify-center rounded-full bg-amber-100"
            >
              <svg
                class="h-10 w-10 animate-pulse text-amber-600"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true"
          ...
          <p v-if="isWithinWorkingHours" class="mt-3 text-center text-gray-600">
            ...
            within <span class="font-bold text-amber-600">5–10 minutes</span> to
            verify your items.
          </p>
          <p v-else class="mt-3 text-center text-gray-600">
            We've received your order! Our office is currently closed. We will
            call you
            <span class="font-bold text-amber-600"
              >first thing tomorrow morning</span
            >
            at
            <span class="font-bold text-gray-900">{{
              order.contact_phone
        ...
        <!-- Real-time Status -->
        <div
          class="rounded-xl bg-blue-50 p-4 text-center"
          role="status"
          aria-live="polite"
        >
          <div
            class="flex items-center justify-center gap-2 text-sm text-blue-800"
          >
            <svg
      ...
      <div v-else-if="isConfirmedStatus" class="space-y-6">
        <div class="rounded-2xl bg-white p-6 shadow-sm">
          ...
          <p class="mt-3 text-center text-gray-600">
            Your order has been verified and is now being prepared for
            {{ order.delivery_method === "pickup" ? "pickup" : "delivery" }}.
          </p>

          <!-- Packaging Status -->
          <div
            class="mt-6 rounded-xl border-2 border-green-200 bg-green-50 p-4"
          >
            <div class="flex items-center gap-3">
              <svg
                class="h-6 w-6 animate-bounce text-green-600"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true"
              ...
              <div>
                ...
                <p class="text-sm text-green-700">
                  We're carefully packing your items
                </p>
              </div>
            </div>
          </div>
    ...
    <Teleport to="body">
      ...
      >
        ...
        >
          ...
          >
            <h3 id="cancel-modal-title" class="text-lg font-bold text-gray-900">
              Cancel Order?
            </h3>
...
const statusLabel = computed(() => {
  ...
  const fulfillmentType =
    order.value?.delivery_method === "delivery" ? "delivery" : "pickup";
  ...
  const labels: Record<string, string> = {
    pending: "Pending",
    awaiting_call: "Awaiting Verification Call",
    processing: "Packing",
    paid: "Paid",
    confirmed: "Verified",
    assigned: "Assigned",
  ...
  return labels[status] || status.replaceAll("_", " ").trim() || "Pending";
});
