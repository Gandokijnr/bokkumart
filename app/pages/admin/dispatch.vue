    ...
    <div class="mx-auto w-full max-w-7xl px-4 py-4 sm:px-6 lg:px-8 lg:py-6">
      <div class="mb-6">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 class="text-2xl font-bold text-gray-900 sm:text-3xl">Dispatch</h1>
            <p class="mt-1 text-sm text-gray-600">Assign ready orders to online drivers. Drivers must accept within 2 minutes.</p>
          </div>
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p class="text-sm font-semibold text-gray-900">Ready for Pickup</p>
              <p class="mt-1 text-xs text-gray-500">Tip: Sort by Area to batch rider trips.</p>
            </div>
          ...
          <div class="mt-4 space-y-3">
            ...
            >
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  <div class="flex items-center gap-2">
                    ...
                    />
                    <p class="font-mono text-xs font-bold text-gray-900">#{{ o.id.slice(-8).toUpperCase() }}</p>
                    <span
                      v-if="isExpired(o)"
                      class="rounded-full bg-red-200 px-2 py-0.5 text-xs font-semibold text-red-800"
                    >
                      Needs reassignment
                    </span>
                  </div>

                  <p class="mt-2 text-sm font-semibold text-gray-900">{{ o.customer_name || 'Customer' }}</p>
                  <p class="mt-1 text-sm text-gray-700">{{ o.area || 'Unknown area' }}</p>
                  <p class="mt-1 text-xs text-gray-500">Status: {{ o.status }}</p>
                </div>
                ...
                <div class="flex flex-col items-end gap-2">
                  ...
                  <button
                    ...
                    :disabled="availableDrivers.length === 0 || selectedOrderIds.size === 0 || actionLoadingId !== null"
              ...
              <div v-if="o.assigned_expires_at" class="mt-3 text-xs text-gray-600">
                Accept deadline: <span class="font-semibold">{{ formatDateTime(o.assigned_expires_at) }}</span>
              </div>
            </div>

            <div v-if="!loading && readyOrders.length === 0" class="rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="mt-1 text-xs text-gray-500">Available + idle drivers can be assigned.</p>
            </div>
            ...
            >
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  ...
                  <p class="mt-1 text-xs text-gray-500">{{ d.phone_number || 'No phone' }}</p>
                </div>
              ...
              <div class="mt-3 text-xs text-gray-600">
                Active assigned orders: <span class="font-semibold">{{ d.active_count }}</span>
              </div>
            </div>
      ...
      <!-- Assign Modal -->
      <div v-if="showAssignModal" class="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center">
        <div class="w-full max-w-lg rounded-2xl bg-white p-5 shadow-xl">
          <div class="flex items-start justify-between">
            <div>
              ...
              <p class="mt-1 text-sm text-gray-600">Assign {{ selectedOrderIds.size }} selected order(s) to a driver.</p>
            </div>
            <button class="rounded-lg p-2 text-gray-500 hover:bg-gray-100" @click="closeAssignModal">✕</button>
          </div>

          <div class="mt-4">
            <label class="mb-2 block text-sm font-medium text-gray-700">Choose driver</label>
            <select
            ...
            >
              ...
              <option v-for="d in availableDrivers" :key="d.id" :value="d.id">
                {{ d.full_name || 'Driver' }} ({{ d.active_count }} active)
              </option>
            </select>
          </div>
          ...
          <p class="mt-3 text-xs text-gray-500">
            Acceptance flow: driver has 2 minutes to accept, otherwise the order is highlighted for reassignment.
          </p>
        </div>
      </div>
...
const parseArea = (o: any) => {
  const addr = o.delivery_details?.address || o.delivery_address?.address || o.delivery_address || null
...
const refreshOrders = async () => {
  const { data, error: ordersError } = await supabase
    .from('orders')
    .select('id, status, delivery_details, delivery_address, driver_id, assigned_expires_at, created_at')
