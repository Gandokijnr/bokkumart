<template>
  <div class="min-h-screen bg-gray-50">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-green-600 mt-1">{{ stats.inStock }}</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-orange-600 mt-1">{{ stats.lowStock }}</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-2xl font-bold text-red-600 mt-1">{{ stats.outOfStock }}</p>
            </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
        <div class="flex flex-wrap items-center gap-4">
          ...
          >
            ...
            <option v-for="store in managedStores" :key="store.id" :value="store.id">
              {{ store.name }}
            </option>
          </select>

          <select v-model="statusFilter" class="rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none focus:ring-2 focus:ring-red-500">
          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="item in filteredInventory" :key="item.id" class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3 text-sm text-gray-700">{{ getStoreName(item.store_id) }}</td>
                <td class="px-4 py-3">
    ...
    <div v-if="showUploadModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h3 class="text-lg font-semibold text-gray-900">Upload Inventory CSV</h3>
            <p class="text-sm text-gray-600">Bulk update your store inventory</p>
          </div>
        ...
        <div class="p-6 space-y-6">
          ...
          <div v-if="managedStores.length > 1">
            ...
            <select v-model="uploadStoreId" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none">
              <option value="">Choose a store...</option>
              <option v-for="store in managedStores" :key="store.id" :value="store.id">{{ store.name }}</option>
            </select>
            <p class="text-xs text-gray-500 mt-1">All products in the CSV will be added to this store</p>
          </div>
          <div v-else-if="managedStores.length === 1" class="bg-gray-50 rounded-lg p-3">
            <p class="text-sm text-gray-700">
              <span class="font-medium">Store:</span> {{ managedStores[0].name }}
            </p>
          </div>
          ...
          <div class="bg-blue-50 rounded-lg p-4">
            <p class="text-sm text-blue-900 mb-2">
              <strong>CSV Format:</strong> name, sku, category, description, price, cost_price, unit, stock_level, store_price, digital_buffer, image_url
            </p>
            <p class="text-xs text-blue-700">
              Required: name, stock_level, price. Store is selected above, not in CSV.
            </p>
            <button
          ...
          >
            ...
            <p class="mt-2 text-sm text-gray-600">
              <span class="font-medium text-red-600 cursor-pointer" @click="$refs.fileInput.click()">Click to upload</span> or drag and drop
            </p>
            <p class="text-xs text-gray-500 mt-1">CSV files only (max 10MB)</p>
            <p v-if="selectedFile" class="mt-2 text-sm font-medium text-green-600">
              Selected: {{ selectedFile.name }}
            </p>
          </div>

          <!-- Upload Results -->
          <div v-if="uploadResult" class="rounded-lg p-4" :class="uploadResult.success ? 'bg-green-50' : 'bg-yellow-50'">
            <p class="font-medium" :class="uploadResult.success ? 'text-green-900' : 'text-yellow-900'">
              {{ uploadResult.success ? 'Upload Successful!' : 'Upload Completed with Issues' }}
            </p>
            <p class="text-sm mt-1" :class="uploadResult.success ? 'text-green-700' : 'text-yellow-700'">
              Processed: {{ uploadResult.processed }} rows | Created: {{ uploadResult.inventoryCreated }} | Updated: {{ uploadResult.inventoryUpdated }} | New Products: {{ uploadResult.productsCreated }}
            </p>
            <div v-if="uploadResult.processingErrors?.length" class="mt-3">
              <p class="text-sm font-medium text-red-700">Errors ({{ uploadResult.processingErrors.length }}):</p>
              <ul class="text-xs text-red-600 mt-1 max-h-32 overflow-y-auto">
                <li v-for="(err, idx) in uploadResult.processingErrors.slice(0, 10)" :key="idx">
                  Row {{ err.row }}: {{ err.message }}
                </li>
                <li v-if="uploadResult.processingErrors.length > 10" class="italic">
                  ... and {{ uploadResult.processingErrors.length - 10 }} more errors
                </li>
              </ul>
            </div>
          </div>
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
          ...
          <button
            @click="uploadFile"
            :disabled="!selectedFile || uploading || (managedStores.length > 1 && !uploadStoreId)"
    ...
    <!-- Manual Entry Modal -->
    <div v-if="showManualEntry" class=" overflow-y-auto fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl max-w-lg w-full">
        ...
        <div class="p-6 space-y-4">
          <div v-if="managedStores.length > 1">
            ...
            <select v-model="manualForm.store_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none">
              <option v-for="store in managedStores" :key="store.id" :value="store.id">{{ store.name }}</option>
            </select>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Digital Buffer</label>
              <input
          ...
          <div>
            ...
            >
              ...
              <p v-if="selectedImageFile" class="mt-1 text-xs text-green-600 font-medium">
          ...
          <div>
            <label class="flex items-center gap-2">
              <input v-model="manualForm.is_visible" type="checkbox" class="rounded border-gray-300 text-red-600 focus:ring-red-500" />
    ...
    <div v-if="editingItem" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl max-w-lg w-full">
        ...
        <div class="p-6 space-y-4">
          ...
          <div>
            <label class="flex items-center gap-2">
              <input v-model="editForm.is_visible" type="checkbox" class="rounded border-gray-300 text-red-600 focus:ring-red-500" />
              <span class="text-sm text-gray-700">Visible to customers</span>
            </label>
          </div>
        </div>
...
const stats = computed(() => {
  ...
  const inStock = inventory.value.filter(i => i.is_visible && i.stock_level > 5).length
  const lowStock = inventory.value.filter(i => i.is_visible && i.stock_level > 0 && i.stock_level <= 5).length
  const outOfStock = inventory.value.filter(i => !i.is_visible || i.stock_level === 0).length
...
const filteredInventory = computed(() => {
  return inventory.value.filter(item => {
    ...
    if (statusFilter.value === 'in_stock') {
      ...
      matchesStatus = item.is_visible && item.stock_level > 0 && item.stock_level <= 5
...
const getStatusClass = (item: any) => {
  if (!item.is_visible || item.stock_level === 0) return 'bg-red-100 text-red-700'
...
const fetchInventory = async () => {
  ...
  if (error) {
    ...
    toast.add({ title: 'Error', description: 'Failed to load inventory', color: 'error' })
  } else {
...
const downloadTemplate = () => {
  const csv = 'name,sku,category,description,price,cost_price,unit,stock_level,store_price,digital_buffer,image_url\n' +
...
const uploadFile = async () => {
  ...
  // Determine store ID
  const storeId = managedStores.value.length === 1
    ? managedStores.value[0].id
    : uploadStoreId.value

  if (!storeId) {
    toast.add({ title: 'Error', description: 'Please select a store', color: 'error' })
    return
  }
  ...
  try {
    ...
    const result = await $fetch('/api/admin/upload-inventory', {
      method: 'POST',
      headers: {
    ...
    if (result.success) {
      toast.add({ title: 'Success', description: 'Inventory uploaded successfully', color: 'success' })
...
const handleImageSelect = (event: Event) => {
  ...
  if (input.files && input.files[0]) {
    ...
    if (file.size > 5 * 1024 * 1024) {
      toast.add({ title: 'Error', description: 'Image must be less than 5MB', color: 'error' })
      return
    }
    ...
    if (uploadError) {
      ...
      if (uploadError.message?.includes('Bucket not found')) {
        ...
        toast.add({ 
          ...
          description: 'RLS policy missing. Run the SQL in BRANCH_MANAGER_CSV.md to fix.', 
          color: 'error',
          timeout: 8000
        })
      } else {
...
const saveManualEntry = async () => {
  ...
  try {
    ...
    toast.add({ title: 'Success', description: `Inventory item ${result.action}`, color: 'success' })
    showManualEntry.value = false
    resetManualForm()
    await fetchInventory()
  } catch (err: any) {
    console.error('saveManualEntry error:', err)
    toast.add({ title: 'Error', description: err?.data?.message || err.message || 'Failed to save item', color: 'error' })
  } finally {
...
const resetManualForm = () => {
  manualForm.value = {
    store_id: managedStores.value.length === 1 ? managedStores.value[0].id : '',
    name: '',
    sku: '',
    description: '',
    price: null,
...
const saveEditItem = async () => {
  ...
  try {
    ...
    toast.add({ title: 'Success', description: 'Item updated', color: 'success' })
    editingItem.value = null
    await fetchInventory()
  } catch (err: any) {
    toast.add({ title: 'Error', description: err.message || 'Failed to update item', color: 'error' })
  }
}
