<template>
  <div>
    ...
    <!-- Manual Entry Modal -->
    <div v-if="showManualModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h3 class="text-lg font-semibold text-gray-900">Add Inventory Item</h3>
            <p class="text-sm text-gray-600">Create or update a product in a store</p>
          </div>
        ...
        <div class="p-6 space-y-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="manualForm.name" type="text" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none" placeholder="e.g. Fresh Cow Milk" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">SKU</label>
              <input v-model="manualForm.sku" type="text" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none" placeholder="e.g. DAIRY-001" />
            </div>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Description</label>
            <textarea v-model="manualForm.description" rows="3" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none" placeholder="Optional description"></textarea>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Price *</label>
              <input v-model.number="manualForm.price" type="number" min="0" step="0.01" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none" placeholder="e.g. 850" />
              <p class="text-xs text-gray-500 mt-1">Required if product does not exist</p>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Stock Level *</label>
              <input v-model.number="manualForm.stock_level" type="number" min="0" step="1" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Unit</label>
              <input v-model="manualForm.unit" type="text" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none" placeholder="unit" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input v-model.number="manualForm.cost_price" type="number" min="0" step="0.01" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Store Price</label>
              <input v-model.number="manualForm.store_price" type="number" min="0" step="0.01" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Digital Buffer</label>
              <input v-model.number="manualForm.digital_buffer" type="number" min="0" step="1" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input v-model="manualForm.image_url" type="url" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:border-red-500 focus:outline-none" placeholder="https://..." />
            </div>
            <div class="flex items-center gap-3 pt-6">
              <input id="manual-visible" v-model="manualForm.is_visible" type="checkbox" class="h-4 w-4" />
              <label for="manual-visible" class="text-sm text-gray-700">Visible in store</label>
            </div>
          </div>
          ...
          <div>
            ...
            <div
              ...
              :class="imageDragOver ? 'border-gray-500 bg-gray-50' : 'border-gray-300'"
              @dragover.prevent="imageDragOver = true"
              @dragleave.prevent="imageDragOver = false"
              @drop.prevent="handleImageDrop"
            >
              <div class="flex items-center gap-4">
                <div class="h-16 w-16 rounded-lg bg-gray-100 overflow-hidden flex items-center justify-center">
                ...
                <div class="flex-1">
                  <p class="text-sm font-medium text-gray-900">
                    {{ selectedImageFile ? selectedImageFile.name : 'Drop an image here' }}
                  </p>
            ...
            <p class="text-xs text-gray-500 mt-2">
              If you upload an image, it will be saved to storage and the Image URL will be filled automatically.
            </p>
          </div>
          ...
          <div class="flex justify-end gap-3 pt-2">
            ...
            <button
              ...
              :disabled="manualSaving || !manualForm.store_id || !manualForm.name || manualForm.stock_level === null"
...
const saveManualEntry = async () => {
  ...
  try {
    ...
    toast.add({ title: 'Success', description: 'Inventory item saved', color: 'success' })
...
const uploadFile = async () => {
  ...
  if (isBranchManager.value) {
    ...
    if (!allowedIds.includes(uploadStoreId.value)) {
      toast.add({ title: 'Error', description: 'Not authorized to upload for this store', color: 'error' })
      return
    }
  }
