...
const blacklistForm = ref({
  type: "manual_verification_required" as
...
// Verify Collection modal (for pickup orders)
const showVerifyCollection = ref(false);
const verifyCollectionOrder = ref<AdminOrder | null>(null);
const verifyCode = ref("");
const verifyError = ref("");
const verifyingCollection = ref(false);

// Verify Delivery PIN modal (for delivery orders)
const showVerifyDeliveryPin = ref(false);
const verifyDeliveryOrder = ref<AdminOrder | null>(null);
...
function openVerifyCollection(order: AdminOrder) {
  verifyCollectionOrder.value = order;
  verifyCode.value = "";
  verifyError.value = "";
...
function closeVerifyCollection() {
  ...
  verifyCollectionOrder.value = null;
  verifyCode.value = "";
  verifyError.value = "";
}
...
async function handleVerifyCollectionCode(code: string) {
  ...
  try {
    ...
    await $fetch("/api/admin/verify-collection", {
      ...
      body: { orderId: verifyCollectionOrder.value.id, code },
    });

    await adminStore.updateOrderStatus(
      verifyCollectionOrder.value.id,
      "delivered",
    );
    closeVerifyCollection();
    toast.add({
      title: "Order Collected",
      description: "Pickup verified and completed",
      color: "success",
    });
  } catch (e: any) {
...
async function handleVerifyDeliveryPinCode(pin: string) {
  ...
  try {
    ...
    await $fetch("/api/orders/verify-delivery-pin", {
      method: "POST",
      headers: { Authorization: `Bearer ${accessToken}` },
      body: { orderId: verifyDeliveryOrder.value.id, pin },
    });

    await adminStore.updateOrderStatus(
      verifyDeliveryOrder.value.id,
      "delivered",
    );
    closeVerifyDeliveryPin();
    toast.add({
...
const pulseCards = computed(() => [
  ...
  },
  {
    ...
    icon: "package",
    color: "blue",
    trend: stats.value?.orders_in_processing
      ? `${stats.value.orders_in_processing} processing`
      : null,
  },
  {
    key: "riders",
    ...
    icon: "truck",
    color: "green",
    trend: stats.value?.orders_out_for_delivery
      ? `${stats.value.orders_out_for_delivery} out for delivery`
      : null,
  },
  {
    key: "revenue",
    ...
    icon: "currency",
    color: "purple",
    trend: stats.value?.cancelled_today
      ? `${stats.value.cancelled_today} cancelled today`
      : null,
  },
]);
          ...
          >
            <div class="flex items-start justify-between">
              <div>
                ...
                <p v-if="card.trend" class="text-xs text-gray-500 mt-1">
                  {{ card.trend }}
                </p>
              </div>
              <div
                :class="`w-10 h-10 rounded-lg bg-${card.color}-100 flex items-center justify-center`"
              >
                <svg
                  class="w-5 h-5"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <p class="text-sm text-gray-600 mt-1">
                  {{ order.delivery_address?.street || "No address" }}
                </p>
              </div>
