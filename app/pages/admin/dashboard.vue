...
// Tab state
const activeTab = ref<'pulse' | 'verification' | 'processing' | 'delivery' | 'completed'>('pulse')

// Pulse stats refresh interval
...
const currentStoreName = computed(() => {
  const store = adminStore.availableStores.find(s => s.id === adminStore.currentStoreId)
...
const filteredOrders = computed(() => {
  switch (activeTab.value) {
    case 'verification':
      return adminStore.pendingVerification
    case 'processing':
...
const pulseCards = computed(() => [
  {
    ...
    value: stats.value?.unconfirmed_orders || 0,
    icon: 'phone',
    color: 'red',
    trend: stats.value?.pending_verification ? `${stats.value.pending_verification} awaiting call` : null
  },
  {
    key: 'pickups',
    label: 'Active Pickups',
    value: stats.value?.active_pickups || 0,
    icon: 'package',
    color: 'blue',
    trend: stats.value?.orders_in_processing ? `${stats.value.orders_in_processing} processing` : null
  },
  {
    key: 'riders',
    label: 'Riders En Route',
    value: stats.value?.riders_en_route || 0,
    icon: 'truck',
    color: 'green',
    trend: stats.value?.orders_out_for_delivery ? `${stats.value.orders_out_for_delivery} out for delivery` : null
  },
  {
    key: 'revenue',
    label: 'Daily Revenue (Confirmed)',
    value: formatCurrency(stats.value?.daily_revenue || 0),
    icon: 'currency',
    color: 'purple',
    trend: stats.value?.cancelled_today ? `${stats.value.cancelled_today} cancelled today` : null
  }
])
...
async function switchBranch(storeId: string | null) {
  ...
  toast.add({
    ...
    description: storeId ? `Viewing ${currentStoreName.value}` : 'Viewing all branches',
    color: 'success'
  })
}
...
async function handleAddRestriction() {
  const restrictionId = await adminStore.addCustomerRestriction(blacklistUserId.value, {
...
async function handleStatusChange(orderId: string, newStatus: string) {
  const success = await adminStore.updateOrderStatus(orderId, newStatus as AdminOrderStatus)
  if (success) {
    toast.add({
...
<template>
  <div class="min-h-screen bg-gray-50">
        ...
        <div class="flex items-center justify-between h-16">
          ...
          <div class="flex items-center gap-3">
            ...
            >
              ...
              <span class="text-sm font-medium text-gray-700">{{ currentStoreName }}</span>
          ...
          <nav class="flex space-x-1 -mb-px">
            <button
              v-for="tab in [
                ...
                { key: 'verification', label: 'Verification Queue', icon: 'phone', count: adminStore.pendingVerification.length },
                { key: 'processing', label: 'Processing', icon: 'package', count: adminStore.processingOrders.length },
                { key: 'delivery', label: 'Delivery', icon: 'truck', count: adminStore.deliveryOrders.length },
            ...
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path v-if="tab.icon === 'activity'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                <path v-else-if="tab.icon === 'phone'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                <path v-else-if="tab.icon === 'package'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                <path v-else-if="tab.icon === 'truck'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0zM13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0" />
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div v-if="activeTab === 'pulse'" class="space-y-6">
          ...
          >
            <div class="flex items-start justify-between">
              <div>
                ...
                <p v-if="card.trend" class="text-xs text-gray-500 mt-1">{{ card.trend }}</p>
              </div>
              <div :class="`w-10 h-10 rounded-lg bg-${card.color}-100 flex items-center justify-center`">
                <svg class="w-5 h-5" :class="`text-${card.color}-600`" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path v-if="card.icon === 'phone'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                  <path v-else-if="card.icon === 'package'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                  <path v-else-if="card.icon === 'truck'" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0zM13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0" />
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200">
          ...
          <div class="divide-y divide-gray-200">
            ...
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                  ...
                  <span :class="`px-2 py-1 text-xs font-medium rounded border ${getStatusColor(order.status)}`">
                    {{ getStatusLabel(order.status) }}
                  </span>
                  <span class="text-sm text-gray-600">{{ order.contact_name || 'Unknown' }}</span>
                  <span class="text-sm text-gray-500">{{ formatCurrency(order.total_amount) }}</span>
                </div>
                <div class="flex items-center gap-2">
                  <span class="text-xs text-gray-500">{{ formatTimeAgo(order.created_at) }}</span>
                  <button
            ...
            <div v-if="filteredOrders.length === 0" class="px-6 py-8 text-center text-gray-500">
      ...
      <div v-else class="space-y-4">
        ...
        >
          ...
          <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                <span v-if="order.claimed_by" class="text-xs text-red-600 font-medium">
                  Claimed
                </span>
          ...
          <div class="p-6">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <p class="text-base font-medium text-gray-900">{{ order.contact_name || 'Unknown' }}</p>
                <p class="text-sm text-gray-600">{{ order.contact_phone }}</p>
                <p class="text-xs text-gray-500 mt-1">User ID: {{ order.user_id?.slice(-8) }}</p>
                <button
              ...
              <div>
                ...
                <p class="text-sm text-gray-900"><span class="font-medium">Total:</span> {{ formatCurrency(order.total_amount) }}</p>
                <p class="text-sm text-gray-600"><span class="font-medium">Subtotal:</span> {{ formatCurrency(order.subtotal) }}</p>
                <p class="text-sm text-gray-600"><span class="font-medium">Service Fee:</span> {{ formatCurrency(order.service_fee || 0) }}</p>
                <p class="text-sm text-gray-600"><span class="font-medium">Delivery Fee:</span> {{ formatCurrency(order.delivery_fee) }}</p>
                <p class="text-sm text-gray-600 mt-1"><span class="font-medium">Payment:</span> {{ order.payment_method === 'pod' ? 'Pay on Delivery' : (order.paystack_reference ? 'Online (Paystack)' : order.payment_method) }}</p>
                <p class="text-sm text-gray-600"><span class="font-medium">Method:</span> {{ order.delivery_method === 'pickup' ? 'Store Pickup' : 'Home Delivery' }}</p>
                <p class="text-sm text-gray-600"><span class="font-medium">Store:</span> {{ order.store?.name || 'Unknown' }}</p>
              </div>
              ...
              <div class="flex flex-col gap-2">
                ...
                <div v-if="order.status === 'pending' || order.status === 'processing'" class="flex gap-2">
                  <button
                  ...
                  <button
                    @click="handleRejectOrder(order.id, 'customer_not_available')"
                ...
                <!-- Delivery Actions -->
                <div v-else-if="order.status === 'paid' || order.status === 'confirmed'" class="flex gap-2">
                  <button
                    @click="openRiderModal(order)"
                    class="flex-1 px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition-colors"
                  >
                    Dispatch Rider
                  </button>
        ...
        <div v-if="filteredOrders.length === 0" class="text-center py-12">
          ...
          <h3 class="text-lg font-medium text-gray-900">No orders in this queue</h3>
    ...
    <div v-if="showBranchSwitcher" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4">
        ...
        <div class="p-6 space-y-2">
          ...
          <button
            ...
            class="w-full px-4 py-3 text-left rounded-lg hover:bg-gray-100 transition-colors"
            :class="{ 'bg-red-50 text-red-700': adminStore.currentStoreId === store.id }"
          >
    ...
    <!-- Verification Modal -->
    <div v-if="showVerifyModal && verifyingOrder" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl shadow-xl max-w-lg w-full mx-4">
        <div class="px-6 py-4 border-b border-gray-200">
          <h3 class="text-lg font-semibold text-gray-900">Verify Order</h3>
          <p class="text-sm text-gray-600">#{{ verifyingOrder.id.slice(-6).toUpperCase() }}</p>
        </div>
        <div class="p-6 space-y-4">
          <div class="space-y-3">
            <label class="flex items-start gap-3">
              ...
              />
              <div>
                <span class="font-medium text-gray-900">Delivery Address Verified</span>
                <p class="text-sm text-gray-500">Customer confirmed the delivery address</p>
              </div>
            </label>
            <label class="flex items-start gap-3">
              <input
              ...
              />
              <div>
                <span class="font-medium text-gray-900">Total Amount Verified</span>
                <p class="text-sm text-gray-500">Customer confirmed the order total</p>
              </div>
            </label>
            <label class="flex items-start gap-3">
              <input
              ...
              />
              <div>
                <span class="font-medium text-gray-900">Substitutions Verified</span>
                <p class="text-sm text-gray-500">Discussed alternative products if needed</p>
              </div>
            </label>
          </div>

          <div v-if="verificationForm.verifiedSubstitutions" class="pl-7 space-y-2">
            <label class="flex items-center gap-2">
              ...
              />
              <span class="text-sm text-gray-700">Customer approved substitutions</span>
            </label>
            <textarea
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex gap-3">
          <button
            ...
            :disabled="!verificationForm.verifiedAddress || !verificationForm.verifiedAmount"
    ...
    <!-- Rider Dispatch Modal -->
    <div v-if="showRiderModal && riderOrder" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4">
        <div class="px-6 py-4 border-b border-gray-200">
          <h3 class="text-lg font-semibold text-gray-900">Dispatch Rider</h3>
          <p class="text-sm text-gray-600">Order #{{ riderOrder.id.slice(-6).toUpperCase() }}</p>
        </div>
        <div class="p-6 space-y-4">
          <div>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Rider Phone</label>
            <input
    ...
    <!-- Blacklist Modal -->
    <div v-if="showBlacklistModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4">
        <div class="px-6 py-4 border-b border-gray-200">
          <h3 class="text-lg font-semibold text-gray-900">Flag Customer</h3>
          <p class="text-sm text-gray-600">Add restriction to customer account</p>
        </div>
        <div class="p-6 space-y-4">
          <div>
            ...
            <select
              ...
              class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-red-500"
            >
              <option value="pod_disabled">Disable Cash on Delivery</option>
              <option value="account_suspended">Suspend Account</option>
              <option value="order_limit">Order Limit</option>
              <option value="manual_verification_required">Require Manual Verification</option>
            </select>
          </div>
          <div>
