<template>
  <div class="min-h-screen bg-gray-50">
    ...
    <main class="mx-auto max-w-7xl px-4 py-4 pb-32 sm:px-6 lg:py-8 lg:pb-8">
      ...
      <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900 sm:text-3xl">
          Shopping Cart
          <span v-if="cartStore.cartCount > 0" class="ml-2 text-lg font-medium text-gray-500">
      ...
      <div v-if="cartStore.items.length === 0" class="flex flex-col items-center justify-center py-16">
        <div class="mb-6 flex h-32 w-32 items-center justify-center rounded-full bg-gray-100">
          <svg class="h-16 w-16 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
          </svg>
        </div>
        <h2 class="mb-2 text-xl font-bold text-gray-900">Your cart is empty</h2>
        ...
        <button
          ...
          class="mb-12 inline-flex items-center justify-center rounded-xl px-8 py-3.5 text-sm font-bold text-white shadow-lg transition-all hover:shadow-xl"
        ...
        <div v-if="categories.length > 0" class="w-full max-w-4xl">
          <h3 class="mb-4 text-center text-lg font-semibold text-gray-900">Popular Categories</h3>
          <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
            <button
              v-for="category in categories.slice(0, 8)"
              :key="category.id"
              @click="navigateTo(`/category/${category.slug}`)"
              class="group flex flex-col items-center rounded-xl border-2 border-gray-200 bg-white p-4 transition-all hover:border-red-600 hover:shadow-md"
            >
              <div class="mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-gray-100 transition-colors group-hover:bg-red-50">
                <span class="text-3xl">{{ getCategoryIcon(category) }}</span>
              </div>
              <span class="text-center text-sm font-medium text-gray-700 group-hover:text-red-600">{{ category.name }}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <!-- Cart Content -->
      <div v-else class="grid gap-6 lg:grid-cols-[1fr,380px] lg:items-start">
        <!-- Left Column: Items -->
        <div class="space-y-4">
          <!-- Store Context Header -->
          <div class="flex items-center gap-4 rounded-xl border-2 p-4" style="border-color: #DC262630; background-color: #DC262608;">
            <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl text-white" style="background-color: #DC2626;">
              <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
              </svg>
            </div>
            <div class="flex-1 min-w-0">
              <p class="font-bold text-gray-900">Shopping from: {{ cartStore.currentStoreName || 'HomeAffairs Store' }}</p>
              <p class="text-sm text-gray-600">All items are reserved for 10 minutes</p>
            </div>
            <div class="hidden sm:block">
              <span
                v-if="reservationTimeRemaining > 0"
              ...
              >
                ...
                {{ formatTime(reservationTimeRemaining) }}
              </span>
            </div>
          </div>
          ...
          <div class="space-y-4">
            <div
              ...
              :key="item.id"
              class="relative rounded-xl border-2 border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md sm:p-5"
              :class="{ 'opacity-60': item.quantity > item.max_quantity }"
            >
              <!-- Stock Warning Badge -->
              <div
                v-if="item.quantity > item.max_quantity"
                class="absolute -top-2 left-4 rounded-full px-3 py-1 text-xs font-bold text-white shadow-md"
                style="background-color: #dc2626;"
              >
                Only {{ item.max_quantity }} in stock at {{ cartStore.currentStoreName }}
              </div>
              ...
              <div class="flex gap-4">
                ...
                <div class="flex flex-1 flex-col justify-between">
                  <div>
                    <h3 class="line-clamp-2 font-semibold text-gray-900 sm:text-lg">{{ item.name }}</h3>
                    <p class="mt-1 text-sm text-gray-500">{{ formatPrice(item.price) }} per unit</p>
                  </div>

                  <div class="mt-4 flex items-center justify-between">
                    <!-- Quantity Controls - Thumb Friendly (44x44px minimum) -->
                    <div class="flex items-center rounded-lg border-2 border-gray-200">
                      <button
                        @click="updateQuantity(item.id, item.quantity - 1)"
                        class="flex h-11 w-11 items-center justify-center text-gray-600 transition-colors hover:bg-gray-100 active:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-50 sm:h-12 sm:w-12"
                      ...
                      <span class="flex h-11 w-11 items-center justify-center font-semibold text-gray-900 sm:h-12 sm:w-12">
                        {{ item.quantity }}
                      </span>
                      <button
                        @click="updateQuantity(item.id, item.quantity + 1)"
                    ...
                    <!-- Item Total -->
                    <p class="font-bold text-gray-900 sm:text-lg">
                ...
                <button
                  ...
                  class="ml-2 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full text-gray-400 transition-colors hover:bg-red-50 hover:text-red-500"
          ...
          <button
            @click="navigateTo('/#categories')"
            class="flex items-center gap-2 font-medium transition-colors hover:text-red-600"
            style="color: #DC2626;"
          >
          ...
          <!-- Cross-Sell Section -->
          <div class="mt-8 rounded-xl border-2 border-gray-200 bg-white p-5">
            <h3 class="mb-4 flex items-center gap-2 font-bold text-gray-900">
              <svg class="h-5 w-5" style="color: #DC2626;" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex gap-4 overflow-x-auto pb-2">
              <div
                ...
                :key="product.id"
                class="flex w-36 flex-shrink-0 flex-col rounded-lg border border-gray-200 bg-white p-3 transition-shadow hover:shadow-md"
              >
                <img
                ...
                <p class="mb-2 text-sm font-semibold" style="color: #DC2626;">{{ formatPrice(product.price) }}</p>
                <button
                  @click="addCrossSellItem(product)"
                  class="mt-auto rounded-lg py-2 text-xs font-bold text-white transition-colors"
        ...
        <div class="lg:sticky lg:top-4">
          <div class="rounded-xl border-2 border-gray-200 bg-white p-5 shadow-sm">
            <h2 class="mb-5 text-lg font-bold text-gray-900">Order Summary</h2>
            ...
            <div class="space-y-3">
              ...
              <div class="border-t-2 border-gray-100 pt-3">
                <div class="flex justify-between">
                  ...
                  <span class="text-xl font-bold" style="color: #DC2626;">{{ formatPrice(totalWithService) }}</span>
                </div>
            ...
            <div
              ...
              class="mt-5 rounded-lg border p-3 text-sm"
            ...
            <div class="mt-5">
              ...
              <textarea
                v-model="orderNote"
                rows="3"
                placeholder="e.g., Please pick firm tomatoes, Call upon arrival at the estate gate..."
                class="w-full resize-none rounded-lg border-2 border-gray-200 p-3 text-sm focus:border-red-600 focus:outline-none"
              ></textarea>
            </div>
            ...
            >
              ...
              <span>Some items are out of stock at {{ cartStore.currentStoreName }}. Please adjust quantities to proceed.</span>
            </div>
            ...
            <button
              ...
              class="mt-5 w-full rounded-xl py-4 text-sm font-bold shadow-lg transition-all hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50"
              style="background-color: #DC2626; color: #fff;"
            >
    ...
    <div
      v-if="cartStore.items.length > 0"
      class="fixed bottom-0 left-0 right-0 border-t-2 border-gray-200 bg-white p-4 shadow-lg lg:hidden"
    >
      ...
      <button
        ...
        class="w-full rounded-xl py-3.5 text-sm font-bold transition-opacity disabled:opacity-50"
        style="background-color: #DC2626; color: #fff;"
      >
    ...
    >
      <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
        ...
        <h3 class="mb-2 text-lg font-bold text-gray-900">Clear your cart?</h3>
        <p class="mb-6 text-sm text-gray-600">This will remove all items from your cart. This action cannot be undone.</p>
...
async function loadCrossSellProducts() {
  ...
  const cartCategoryIds = [...new Set(cartStore.items.map(item => item.category_id).filter(Boolean))]
...
function addCrossSellItem(product: typeof crossSellProducts.value[0]) {
  ...
  supabase
    ...
    .single()
    .then(({ data }) => {
      if (data && data.store_inventory && data.store_inventory.length > 0) {
        const storeInfo = data.store_inventory[0]
        const result = cartStore.addItem({
          id: data.id,
          product_id: data.id,
          name: data.name,
          price: storeInfo.store_price || data.price,
...
/* Hide scrollbar for cross-sell but keep functionality */
.overflow-x-auto {
