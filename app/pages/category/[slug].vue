  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <main class="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <div v-else-if="error" class="rounded-2xl border border-red-200 bg-red-50 p-6 text-center">
        ...
        <button @click="fetchCategoryAndProducts" class="mt-4 rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white hover:bg-red-700">
          Try Again
        </button>
      </div>

      <!-- Empty state -->
      <div v-else-if="!category" class="rounded-2xl border-2 border-gray-200 bg-white p-12 text-center">
        <div class="mb-4 text-6xl">📦</div>
        <h3 class="text-lg font-semibold text-gray-900">Category not found</h3>
        <button @click="navigateTo('/#deals')" class="mt-6 rounded-xl bg-red-600 px-6 py-3 font-medium text-white hover:bg-red-700">
          Browse All Products
        </button>
      </div>

      <div v-else-if="products.length === 0" class="rounded-2xl border-2 border-gray-200 bg-white p-12 text-center">
        <div class="mb-4 text-6xl">📦</div>
        <h3 class="text-lg font-semibold text-gray-900">No products found</h3>
        <p class="mt-2 text-sm text-gray-600">No products available in this category.</p>
        <button @click="navigateTo('/#deals')" class="mt-6 rounded-xl bg-red-600 px-6 py-3 font-medium text-white hover:bg-red-700">
          Browse All Products
        </button>
      </div>
...
useHead({
  title: computed(() => category.value ? `${category.value.name} | HomeAffairs` : 'Category | HomeAffairs')
})
</script>
