      ...
      <div class="mb-8 rounded-2xl bg-red-700 p-6 text-white shadow-lg sm:p-8">
        ...
        >
          <div class="flex items-center gap-4">
            <div
              class="flex h-16 w-16 items-center justify-center rounded-full bg-white/20 text-2xl backdrop-blur-sm"
            >
              {{ userInitials }}
            </div>
            <div>
      ...
      <div class="min-h-[400px]">
        ...
        <div v-if="activeTab === 'overview'" class="space-y-6">
          ...
          <div class="grid gap-4 sm:grid-cols-3">
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-lg font-bold text-gray-900">
                    {{
                      lastOrder
                        ? formatOrderDate(lastOrder.created_at)
                        : "No orders yet"
                    }}
                  </p>
                </div>
              ...
              <div v-if="lastOrder" class="mt-3 flex items-center gap-2">
                <span
                  ...
                  :class="
                    getStatusBadge(lastOrder.status, lastOrder.delivery_method)
                      .bg +
                    ' ' +
                    getStatusBadge(lastOrder.status, lastOrder.delivery_method)
                      .color
            ...
            <div
              class="rounded-2xl border-2 border-gray-200 bg-white p-5 shadow-sm"
            >
              <div class="flex items-center gap-3">
                <div
                ...
                <div>
                  ...
                  <p class="text-sm font-medium text-gray-900">
                    {{
                      primaryAddress
                        ? getLabelDisplay(primaryAddress.label)
                        : "No address set"
                    }}
                  </p>
                </div>
              ...
              >
                ...
                <div class="flex items-center gap-3">
                  <span
                    ...
                    :class="
                      getStatusBadge(order.status, order.delivery_method).bg +
                      ' ' +
                      getStatusBadge(order.status, order.delivery_method).color
                    "
                  >
                    {{
          ...
          <div v-else-if="ordersPending" class="space-y-4">
            <div v-for="n in 3" :key="n" class="rounded-2xl bg-gray-100 p-6">
              <div class="flex gap-4">
                <div
                  class="h-16 w-16 animate-pulse rounded-lg bg-gray-200"
                ></div>
                <div class="flex-1 space-y-2">
                  <div
                    class="h-4 w-1/3 animate-pulse rounded bg-gray-200"
                  ></div>
                  <div
                    class="h-4 w-1/4 animate-pulse rounded bg-gray-200"
                  ></div>
                </div>
              </div>
            </div>
          ...
          <div v-else class="space-y-4">
            ...
            >
              ...
              >
                <div class="flex gap-4">
                  <div
                    class="flex h-16 w-16 items-center justify-center rounded-xl bg-gray-100 text-2xl"
                  >
                    📦
                  </div>
                  <div>
                    <div class="flex items-center gap-2">
                      ...
                      <span
                        class="rounded-full px-2.5 py-0.5 text-xs font-medium"
                        :class="
                          getStatusBadge(order.status, order.delivery_method)
                            .bg +
                          ' ' +
                          getStatusBadge(order.status, order.delivery_method)
                            .color
                ...
                <div class="flex gap-2">
                  ...
                  >
                    <span
                      v-if="arrivalPending.has(order.id)"
...
import {
  ...
  type Order,
  type OrderStatus,
  getOrderStepperSteps,
  getStepperKeyForStatus,
...
const saveAddress = async () => {
  ...
  if (editingAddress.value) {
    const { success, error } = await updateAddress(
...
const reportPickupArrival = async (order: Order) => {
  ...
  try {
    ...
    showToast(
      "Notified the store. Head to the Online Pickup Point.",
      "success",
    );
    await fetchOrders();
  } catch (err: any) {
    ...
    showToast(
      err?.data?.message || err?.message || "Failed to notify store",
      "error",
    );
  } finally {
...
onMounted(async () => {
  ...
  subscribeToOrderUpdates((update) => {
    ...
    const storeName =
      (order as any)?.stores?.name || (order as any)?.metadata?.store_name;

    if (fulfillmentType === "pickup" && update.newStatus === "picked_up") {
      showToast(
        `Great news! Your order is ${label}${storeName ? ` at the ${storeName} branch` : ""}.`,
        "info",
      );
      return;
    }
