...
const updatePassword = async () => {
  ...
  try {
    ...
    showNotification('Error', error.message || 'Failed to update password.', 'red')
  } finally {
    loading.value = false
  }
}
...
onMounted(async () => {
  ...
  if (error || !session) {
    ...
    if (recoveryError || !data.user) {
      resetError.value = 'This password reset link is invalid or has expired. Please request a new one.'
    }
  }
})
</script>
