...
export const useAdminStore = defineStore('admin', {
  ...
  getters: {
    ...
    completedOrders: (state) => {
      const filtered = state.orders.filter(o => ['delivered', 'cancelled', 'refunded'].includes(o.status))
  ...
  actions: {
      ...
      switch (payload.eventType) {
        case 'UPDATE':
          ...
          if (index !== -1) {
            ...
            this.orders[index] = { ...this.orders[index], ...payload.new, ...optimistic }
          }
          break
    ...
    async logInteraction(
      ...
      try {
        ...
        if (error) {
          console.warn('log_order_interaction failed (migration may not be applied):', error.message)
    ...
    async rejectOrder(orderId: string, reason: string, details?: string): Promise<boolean> {
      const supabase = useSupabaseClient<Database>()
    ...
    async addCustomerRestriction(
      userId: string,
      restriction: {
        type: 'pod_disabled' | 'account_suspended' | 'order_limit' | 'manual_verification_required'
        reason: string
        details?: string
        expiresAt?: string
      }
    ...
    async removeCustomerRestriction(restrictionId: string): Promise<boolean> {
      ...
      try {
        const { data, error } = await supabase.rpc('remove_customer_restriction', {
    ...
    async updateOrderStatus(orderId: string, newStatus: AdminOrderStatus, notes?: string): Promise<boolean> {
      const supabase = useSupabaseClient<Database>()
      ...
      try {
        ...
        const { error } = await (supabase.from('orders').update as any)(updates)
          .eq('id', orderId)
