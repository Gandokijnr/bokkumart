  ...
  actions: {
    ...
    async addItem(
      ...
      if (this.items.length > 0 && this.currentStoreId !== product.store_id) {
        ...
        return {
          success: false,
          error: `You can only order from one BokkuXpress branch at a time. Your cart contains items from ${existingStoreName}. Clear cart to switch to ${product.store_name}?`,
