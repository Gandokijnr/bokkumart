...
import { useStoreLocator, type StoreWithDistance } from '~/composables/useStoreLocator'
...
const isStoreOpen = (store: Store): boolean => {
  ...
  const hours = store.operating_hours[dayOfWeek as keyof typeof store.operating_hours]
    ...
    <div class="w-full max-w-2xl bg-white rounded-xl shadow-xl overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-600">
            Choose a HomeAffairs location to see available products
          </p>
        </div>
        <button
          @click="close"
      ...
      <div class="px-6 py-4 border-b border-gray-200">
        ...
        <div class="flex gap-3">
          <button
            :class="shoppingMode === 'delivery' ? 'bg-red-600 text-white' : 'bg-white text-gray-700 border border-gray-300'"
          ...
          <button
            :class="shoppingMode === 'pickup' ? 'bg-red-600 text-white' : 'bg-white text-gray-700 border border-gray-300'"
            class="flex-1 rounded-lg px-4 py-2 text-sm font-medium transition-colors flex items-center justify-center gap-2"
      ...
      <div v-if="!storeStore.isStoreSelected" class="px-6 py-3 border-b border-gray-200">
        <button
          :disabled="detectingLocation"
          class="rounded-lg bg-red-100 px-4 py-2 text-sm font-medium text-red-700 hover:bg-red-200 transition-colors disabled:opacity-50 flex items-center gap-2"
        ...
        <p v-if="nearestStore" class="mt-2 text-sm text-green-600 flex items-center gap-1">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          Found nearest store: {{ nearestStore.name }} ({{ nearestStore.distanceFormatted }})
        </p>
      </div>
      ...
      <div class="px-6 py-4 max-h-96 overflow-y-auto">
        <div v-if="loading" class="space-y-4">
          <div v-for="i in 4" :key="i" class="h-24 w-full bg-gray-100 rounded-lg animate-pulse" />
        </div>

        <div v-else-if="error" class="rounded-lg bg-red-50 p-4 text-red-600 flex items-center gap-2">
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1">
                ...
                <div class="mt-2 flex items-center gap-4 text-sm">
                  <span :class="getStoreStatus(store).color" class="flex items-center gap-1">
                    <svg v-if="isStoreOpen(store)" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                ...
                <p v-if="shoppingMode === 'pickup' && store.pickup_instructions" class="mt-2 text-xs text-gray-500 flex items-center gap-1">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
        ...
        >
          {{ storeStore.loading ? 'Loading...' : (shoppingMode === 'delivery' ? 'Continue with Delivery' : 'Continue with Pickup') }}
        </button>
      </div>
    </div>
  </div>
