...
// Local state
const selectedBranchId = ref<string | null>(branchStore.activeBranch?.id || null)
...
const getBranchStatus = (branch: Branch): { text: string; color: string; icon: string } => {
  if (isBranchOpen(branch)) {
    return { text: 'Open Now', color: 'text-green-600 bg-green-50', icon: 'check' }
  }
...
const handleConfirmSelection = async () => {
  ...
  if (cartStore.items.length > 0 && cartStore.currentStoreId !== selectedBranchId.value) {
...
<template>
  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        ...
        >
          <!-- Header -->
          <div class="px-6 py-5 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-red-600 to-red-700">
            <div>
              <h2 class="text-xl font-bold text-white">
                {{ branchStore.hasActiveBranch ? 'Switch Store' : 'Welcome to HomeAffairs' }}
              </h2>
          ...
          <div class="flex-1 overflow-y-auto">
            ...
            <div v-if="branchStore.loading" class="p-6 space-y-4">
              <div v-for="i in 3" :key="i" class="h-24 w-full bg-gray-100 rounded-xl animate-pulse" />
            </div>

            <!-- Error State -->
            <div v-else-if="branchStore.error || error" class="p-6">
              <div class="rounded-xl bg-red-50 p-4 text-red-600 flex items-center gap-3">
              ...
              >
                <div class="flex items-start gap-4">
                  ...
                  <div 
                    ...
                    :class="selectedBranchId === branch.id ? 'border-red-600 bg-red-600' : 'border-gray-300'"
                  >
                    <svg 
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                      ...
                      >
                        <svg v-if="getBranchStatus(branch).icon === 'check'" class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                        </svg>
          ...
          <div class="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-end gap-3">
            ...
            >
              ...
              <span>{{ isLoading ? 'Switching...' : (branchStore.hasActiveBranch ? 'Switch Store' : 'Start Shopping') }}</span>
            </button>
          </div>
        </div>
      </div>
  ...
  <Teleport to="body">
    ...
    >
      <div v-if="showCartWarning" class="fixed inset-0 z-[110] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          ...
          <div class="p-6">
            <div class="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p class="text-sm text-gray-600">This action will affect your cart</p>
              </div>
            </div>
            ...
            <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
              ...
              <p class="text-sm text-amber-800 mt-2">
                Switching to a different store will <span class="font-bold">clear your current cart</span>. 
                You can only order from one HomeAffairs branch at a time.
              </p>
            </div>
