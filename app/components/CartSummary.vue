  ...
  <div class="rounded-xl border-2 border-gray-200 bg-white p-4 shadow-sm sm:p-5">
    ...
    <div class="space-y-2.5 sm:space-y-3">
      ...
      <div class="flex justify-between text-xs sm:text-sm">
        ...
        <span v-if="deliveryCalculated" class="font-medium text-gray-900">{{ formatPrice(deliveryFee) }}</span>
        <button
          v-else
    ...
    >
      <div class="flex items-start gap-2">
        <svg class="mt-0.5 h-4 w-4 flex-shrink-0 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
        </svg>
        <div>
          <p class="font-medium text-gray-900">
            {{ deliveryDetails.method === 'pickup' ? 'Store Pickup' : 'Home Delivery' }}
          </p>
    ...
    <div class="mt-4 sm:mt-5">
      ...
      <textarea
        :value="orderNote"
        @input="$emit('update:orderNote', ($event.target as HTMLTextAreaElement).value)"
        rows="3"
