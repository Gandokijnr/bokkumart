...
const filteredLogs = computed(() => {
  ...
  if (filterActionType.value !== 'all') {
    filtered = filtered.filter(log => log.action_type === filterActionType.value)
  }
...
const getActionIcon = (actionType: string) => {
  const icons: Record<string, string> = {
    price_change: 'M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
    inventory_update: 'M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4',
    stock_adjustment: 'M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z',
    product_visibility_change: 'M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z',
    manager_assignment: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z'
  }
...
<template>
  <div class="space-y-4">
    ...
    <div v-if="!entityId" class="bg-white rounded-lg border border-gray-200 p-4">
    ...
    <div class="bg-white rounded-lg border border-gray-200">
      <div class="px-6 py-4 border-b border-gray-200">
        ...
        <p class="text-sm text-gray-500">Activity log of all changes and actions</p>
      </div>
      ...
      <div v-else-if="filteredLogs.length === 0" class="px-6 py-12 text-center text-gray-500">
      ...
      <div v-else class="divide-y divide-gray-200">
        ...
        >
          <div class="flex items-start gap-4">
            <!-- Icon -->
            <div :class="`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${getActionColor(log.action_type)}`">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-start justify-between gap-4">
                ...
                <!-- Action Type Badge -->
                <span :class="`inline-flex px-2 py-1 text-xs font-medium rounded ${getActionColor(log.action_type)}`">
              ...
              <details v-if="log.old_value || log.new_value" class="mt-2">
                ...
                <div class="mt-2 p-3 bg-gray-50 rounded border border-gray-200 text-xs space-y-2">
                  <div v-if="log.old_value">
                    <span class="font-medium text-gray-700">Before:</span>
                    <pre class="mt-1 text-gray-600 overflow-x-auto">{{ formatValue(log.old_value) }}</pre>
                  </div>
                  <div v-if="log.new_value">
                    <span class="font-medium text-gray-700">After:</span>
                    <pre class="mt-1 text-gray-600 overflow-x-auto">{{ formatValue(log.new_value) }}</pre>
                  </div>
                  <div v-if="log.metadata && Object.keys(log.metadata).length > 0">
                    <span class="font-medium text-gray-700">Metadata:</span>
                    <pre class="mt-1 text-gray-600 overflow-x-auto">{{ formatValue(log.metadata) }}</pre>
                  </div>
                </div>
              </details>
            </div>
