<template>
  <div v-if="products.length > 0" class="mt-6 rounded-xl border-2 border-gray-200 bg-white p-4 sm:mt-8 sm:p-5">
    <h3 class="mb-3 flex items-center gap-2 text-sm font-bold text-gray-900 sm:mb-4 sm:text-base">
      <svg class="h-4 w-4 text-red-600 sm:h-5 sm:w-5" fill="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex gap-3 overflow-x-auto pb-2 sm:gap-4">
      ...
      >
        ...
        <p class="mb-2 text-xs font-semibold text-red-600 sm:text-sm">{{ formatPrice(product.price) }}</p>
        <button
          @click="$emit('add', product)"
          class="mt-auto rounded-lg bg-red-600 py-1.5 text-xs font-bold text-white transition-colors sm:py-2"
        >
          Add to Cart
        </button>
      </div>
