...
export const useAdminData = (options: AdminDataOptions = {}) => {
  ...
  const isBranchManager = computed(() => profile.value?.role === 'branch_manager')
  const managedStoreIds = computed(() => profile.value?.managed_store_ids || [])

  const currentStoreNames = computed(() => {
    if (managedStores.value.length === 0) return 'No Stores Assigned'
    if (managedStores.value.length === 1) return managedStores.value[0].name
    return managedStores.value.map(s => s.name).join(', ')
  })
  ...
  const initialize = async () => {
    ...
    try {
      ...
      if (profileData.managed_store_ids && profileData.managed_store_ids.length > 0) {
        const { data: storesData, error: storesError } = await supabase
          .from('stores')
          .select('*')
          .in('id', profileData.managed_store_ids)
   ...
   */
  const fetchInventory = async (additionalFilters: Record<string, any> = {}) => {
    let query = supabase
  ...
  const fetchDashboardStats = async () => {
    ...
    const storeIdParam = isSuperAdmin.value ? null : managedStoreIds.value[0] || null

    const { data, error: statsError } = await supabase
      .rpc('get_admin_dashboard_stats', { p_store_id: storeIdParam })
