...
export const useAuditLog = () => {
  ...
  const logPriceChange = async (
    ...
    try {
      ...
      const { data, error } = await (supabase.rpc as any)("log_audit_action", {
        ...
        p_metadata: {
          ...
          percentage_change: (((newPrice - oldPrice) / oldPrice) * 100).toFixed(
            2,
          ),
        },
      });
  ...
  const logManagerAssignment = async (
    ...
    try {
      ...
      const { data: stores } = (await supabase
        .from("stores")
        .select("id, name")
        .in("id", [...oldStoreIds, ...newStoreIds])) as {
        data: { id: string; name: string }[] | null;
      };

      const storeMap = new Map(stores?.map((s) => [s.id, s.name]) || []);

      const oldStoreNames = oldStoreIds
        .map((id) => storeMap.get(id))
        .filter(Boolean)
        .join(", ");
      const newStoreNames = newStoreIds
        .map((id) => storeMap.get(id))
        .filter(Boolean)
        .join(", ");
      ...
      const { data, error } = await (supabase.rpc as any)("log_audit_action", {
        ...
        p_old_value: {
          managed_store_ids: oldStoreIds,
          store_names: oldStoreNames,
        },
        p_new_value: {
          managed_store_ids: newStoreIds,
          store_names: newStoreNames,
        },
  ...
  const fetchAuditLogs = async (
    ...
    try {
      let query = supabase
        .from("audit_logs")
        .select(
          "id, user_id, user_name, action_type, entity_type, entity_id, store_id, store_name, old_value, new_value, description, metadata, created_at",
        )
