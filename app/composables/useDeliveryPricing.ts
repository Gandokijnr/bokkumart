import { calculateDistance } from './useStoreLocator'
...
const ISLAND_AREAS = ['ikoyi', 'vi', 'victoria island', 'lekki', 'banana island', 'maroko']

// Check if address is on Lagos Island
export function isIslandLocation(address: string): boolean {
  const lowerAddress = address.toLowerCase()
  return ISLAND_AREAS.some(area => lowerAddress.includes(area))
}

// Check if current time is peak traffic hour in Lagos
export function isPeakTrafficHour(): boolean {
  const now = new Date()
...
export function calculateDeliveryFee(
  ...
  let baseFee = storeIsIsland || deliveryIsIsland 
  ...
  if (isPeakTrafficHour()) {
    trafficSurcharge = Math.round((baseFee + distanceFee) * (LAGOS_DELIVERY_CONFIG.trafficMultiplier - 1))
  }

  // Island surcharge for cross-mainland/island deliveries
  let islandSurcharge = 0
  if ((storeIsIsland && !deliveryIsIsland) || (!storeIsIsland && deliveryIsIsland)) {
...
export function calculatePickupTime(store: Store): string {
  const now = new Date()
...
export async function getZoneBasedDeliveryFee(
  ...
  const matchingZone = zones.find(
    (zone: { min_distance_km: number; max_distance_km: number; delivery_fee: number }) => distance >= zone.min_distance_km && distance <= zone.max_distance_km
  )
...
export const useDeliveryPricing = () => {
  ...
  const estimateDeliveryTime = (store: Store, deliveryAddress: DeliveryAddress): string => {
